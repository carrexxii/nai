...
let
    ...
    debug_flags = &"--cc:tcc --mm:arc --passL:\"{linker_flags}\" --nimCache:{build_dir} -o:{bin_path} " &
                  &"--passL:\"-ldl -lm\" --tlsEmulation:on -d:useMalloc"
    release_flags = &"--cc:gcc --mm:arc --passL:\"{linker_flags}\" --nimCache:{build_dir} -o:{bin_path} -d:release -d:danger -d:lto --opt:speed"
    post_release = @[""]
